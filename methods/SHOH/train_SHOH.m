...
for i=1:num_class1
    ...
    cnt=numel(ddx);
    if cnt>0
        T{1,8}(:,i)= (T{1,8}(:,i)*train.cnt1(i)+sum(X_new(:,ddx),2))./(train.cnt1(i)+numel(ddx));
        T{1,10}(:,i)= (T{1,10}(:,i)*train.cnt1(i)+sum(Y_new(:,ddx),2))./(train.cnt1(i)+numel(ddx));
...
for i=1:num_class2
    ...
    cnt=numel(ddx);
    if cnt>0
        T{1,9}(:,i)=(T{1,9}(:,i)*train.cnt2(i)+sum(X_new(:,ddx),2))./(train.cnt2(i)+numel(ddx));
        T{1,11}(:,i)= (T{1,11}(:,i)*train.cnt2(i)+sum(Y_new(:,ddx),2))./(train.cnt2(i)+numel(ddx));
